...
class GrupoController{
    ...
    grupoCreatepage(req: Request, res: Response){
        res.render('creategrupo')
    }

    grupoDeletepage(req: Request, res: Response){
        res.render('deletegrupo')
    }
    ...
    async createGrupo(req: Request, res: Response){
        ...
        const dataapresent = new Date(diaApresentacao)
        const estadanen: number = parseInt(estande)
        const novaLinha = await prisma.grupo.create({
        data: {
            nome,
            liderNome,
            estande: estadanen,
            diaApresentacao: dataapresent
        },
        });
        res.json({
            novaLinha
